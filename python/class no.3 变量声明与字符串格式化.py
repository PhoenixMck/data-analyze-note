...
# 变量命名背后的逻辑
"""
不同于其他语言，python是把名字【变量】放在值上，类似于标签而不是把值存储在变量中
修改名字意味着创建一个新的变量，给值更换了标签，实际上并没有改变值
同一个值可以有多个标签
修改值意味着创建一个全新的东西，并把原有的标签贴上去，而原有的值则失去他的标签，如果它上面还有标签，那么它仍会保留在内存中，否则它将被删除
"""
# 复制应使用函数，直接使用a=b只是做标签的新增，实际值并没有增加，修改a同时也会修改b的
a = [1, 2, 3]
b = a.copy()  # 如list要使用copy
print(b)
a = 1
b = a
b += 1  # 这里是把b的标签赋予了新值，并不是修改值，所以不影响a
print(a)
# 命名规范：区分大小写，无空格，不允许数字开头
# 标识符是对对象引用起的一个名字。
# 格式化字符串【两种占位符%与{}】
# %占位符 函数/把值插入到一个有字符串格式符%s（%s s会被后续的变量按顺序替代）的字符串中,代表强制类型转换
k = 1
v = 2
...
#%的具体参数：%[(name][flags对齐方式<^>，默认右对齐，-也表示左对齐，不是负号噢][width宽度，小数点也算一位][.precision精度，会自动四舍五入]typecode
...
# 字符串的strip（）函数去掉字符串头尾指定的内容，参数可为字符串，即要删除的内容【只要字符串里的字符有即删除，不用按顺序，不是要求完全匹配】，默认为空格，包括'\n', '\r',  '\t',  ' '，但是strip会返回一个字符串，实际上不改变原字符串的内容
a = " a  1 "
print(a.strip())
print(a)
